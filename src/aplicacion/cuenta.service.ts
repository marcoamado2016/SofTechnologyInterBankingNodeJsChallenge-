...
import Cuenta, {
  ICuentaSchema,
} from "../infrastructura/repositorio/schema/cuentaModel";
...
} from "../infrastructura/repositorio/schema/transferenciaModel";
import { mesActualYUltimo } from "../compartido/fechas";

@injectable()
export class CuentaService {
  constructor(
    @inject(TYPES.CuentaRepositorio)
    private cuentaRepositorio: ICuentaRepositorio
  ) {}
  async crearCuentaBancariaService(
    cuenta: CuentaEntity,
    res: Response
  ): Promise<Respuesta> {
    ...
    return await this.cuentaRepositorio.crearCuentaBancariaRepositorio(
      newCuenta
    );
  }
  async transferenciaService(
    ...
    return await this.cuentaRepositorio.transferenciaRepositorio(
      newTransferencia
    );
  }
  async empresaTransferenciaUltimoMesService(): Promise<Respuesta> {
    const { mesActual, ultimoMes } = mesActualYUltimo();
    return await this.cuentaRepositorio.empresaTransferenciaUltimoMesRepositorio(
      mesActual,
      ultimoMes
    );
  }
