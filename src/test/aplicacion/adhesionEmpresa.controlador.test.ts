...
describe("Adhesion controller", () => {
  ...
  it("devuelve 201 si la adhesion es exitosa", async () => {
    ...
    const adhesionEmpresaMock =
      _empresa.AdhesionEmpresaService as MockedFunction<
        (body: Cuerpo, res: Response) => Promise<Respuesta>
      >;
    adhesionEmpresaMock.mockResolvedValue(respuesta);
    await AdhesionEmpresaController(req as Request, res as Response);
    expect(_empresa.AdhesionEmpresaService).toHaveBeenCalledWith(req.body, res);
