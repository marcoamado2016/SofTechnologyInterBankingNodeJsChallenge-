...
import { _empresa, AdhesionEmpresaultimoMes } from "../../infrastructura/controllers/empresaController";
...
describe('Adhesion empresa ultimo mes ', () => {
    ...
    it('adhesion ultimo mes ok', async () => {
        ...
        const adhesionEmpresasUltimoMesMock = _empresa.AdhesionEmpresaultimoMes as MockedFunction<() => Promise<Respuesta>>;
        adhesionEmpresasUltimoMesMock.mockResolvedValue(respuesta);
        await AdhesionEmpresaultimoMes(res as Response)
        expect(_empresa.AdhesionEmpresaultimoMes).toHaveBeenCalledWith();
