...
export const CuentaBancariaController = async (req: Request, res: Response) => {
  try {
    const response: Respuesta = await _cuenta.crearCuentaBancariaService(
      req.body as CuentaEntity,
      res
    );
...
export const empresaTransferenciaUltimoMesController = async (
  req: Request,
  res: Response
) => {
  try {
    const response: Respuesta =
      await _cuenta.empresaTransferenciaUltimoMesService();
    res.status(HTTP_CODES.OK).send(response);
  } catch (error: any) {
    res.status(HTTP_CODES.INTERNAL_ERROR).json({ error: error.message });
  }
