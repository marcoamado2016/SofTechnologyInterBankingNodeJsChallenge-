...
import Transferencia, {
  ITransferenciaSchema,
} from "../schema/transferenciaModel";

export class CuentaMongoRepositorio implements ICuentaRepositorio {
  async crearCuentaBancariaRepositorio(
    newCuenta: ICuentaSchema
  ): Promise<Respuesta> {
    try {
      ...
      if (cuentaEncontrada)
        throw new Error(
          `Ya existe una cuenta registrada con este numero: ${numeroCuenta}`
        );
  ...
  async transferenciaRepositorio(
    transferencia: ITransferenciaSchema
  ): Promise<Respuesta> {
    try {
      const cuentaOrigen = await Cuenta.findOne({
        numeroCuenta: transferencia.numeroCuentaOrigen,
      });
      const cuentadestino = await Cuenta.findOne({
        numeroCuenta: transferencia.numeroCuentaDestino,
      });
      if (!cuentaOrigen) throw new Error("No existe la cuenta origen");
      if (!cuentadestino) throw new Error("No existe la cuenta destino");
      if (transferencia.importe > cuentaOrigen.saldo)
        throw new Error(
          "Saldo de la cuenta origen es inferior al importe a transferir"
        );
      ...
      transferencia.save();
      return new Respuesta("transferencia OK", {
  ...
  async empresaTransferenciaUltimoMesRepositorio(
    mesActual: Date,
    ultimoMes: Date
  ): Promise<Respuesta> {
    try {
      ...
      return new Respuesta(
        "Transferencia ultimo mes",
        transferenciaAdheridasUltimoMes
      );
    } catch (error) {
      throw new Error(error as string);
    }
